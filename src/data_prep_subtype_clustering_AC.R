...
#sigDir= "data/Signatures/"
sigDir="data"

barcodes <- read.table(file.path(sigDir,"EBpp_whitelist_no_blood_cancers.csv"), sep=",", header=T, stringsAsFactors=F)
...
res0 <- read.table(file.path(sigDir,"ssGSEA_Scores_Wolf68_Bindea_Yasin_C7.tsv.gz"), sep="\t", header=T, stringsAsFactors=T)
...
## Wolf Scores
wolf <- read.table(file.path(sigDir,"TCGA_pancancer_10852whitelistsamples_68ImmuneSigs.csv"), sep=",", header=T, stringsAsFactors=F)
wolfNames <- wolf$X
...
## Attractors
anastRNA <- read.table(file.path(sigDir,"mRNA_immune_attractors_EBpp_whitelist.tsv"), sep="\t", header=T, stringsAsFactors=F)
anastNames <- rownames(anastRNA)

## Cibersort scores cell types
ciber <- read.table(file.path(sigDir,"TCGA.Kallisto.cibersort.relative.tsv"), sep="\t", header=T, stringsAsFactors=F)
...
#c7codes <- read.table(file.path(sigDir,"c7_dict_learning/modl_sparse_codes.txt", sep="\t", header=F)
...
c7atoms <- read.table(file.path(sigDir,"c7_dict_transpose/modl_atoms.txt"), sep="\t", header=F)
...
## ICR
icr <- read.table(file.path(sigDir,"ICR_Scores.csv"), sep=",", header=T, stringsAsFactors=F)
